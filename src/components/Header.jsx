...
const Header = () => {
  ...
  const navLinkStyle = ({ isActive }) => ({ color: isActive ? '#5DE2E7' : '#FFFFFF' });
  const mobileNavLinkStyle = ({ isActive }) => ({ backgroundColor: isActive ? 'rgba(93, 226, 231, 0.1)' : 'transparent', color: isActive ? '#5DE2E7' : '#FFFFFF' });
            ...
            <div className="hidden lg:flex items-center">
                <nav className="flex items-center space-x-7">
                    ...
                    <motion.div className="relative" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}>
                        <NavLink to="/services" style={navLinkStyle} className="text-sm font-semibold hover:text-accent flex items-center gap-1 transition-colors">
                            Services
                ...
                <div className="flex items-center ml-8">
                    <div className="flex items-center space-x-4">
                        {socialLinks.map(link => ( <a key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" className="text-white hover:text-accent transition-colors"> <span className="sr-only">{link.name}</span> {link.icon} </a> ))}
                    </div>
