...
const servicesDropdownLinks = [
    ...
    { path: '/services/payroll', name: 'Payroll Administrators' },
    ...
    { path: '/services/website-design', name: 'Website Design & Hosting' },
];
...
const Header = () => {
  ...
  const navLinkStyle = ({ isActive }) => ({ color: isActive || (isServicesActive && event.target?.pathname?.startsWith('/services')) ? '#5DE2E7' : '#FFFFFF' });
  
  const mobileNavLinkStyle = (isActive) => ({
    backgroundColor: isActive ? 'rgba(93, 226, 231, 0.1)' : 'transparent',
    color: isActive ? '#5DE2E7' : '#FFFFFF',
  });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <Link to="/" className="flex-shrink-0 flex items-center">
              ...
              <span className="font-unbounded text-xl font-bold text-white ml-3">
            ...
            <div className="hidden lg:flex items-center">
                <nav className="flex items-center space-x-7">
                    <NavLink to="/" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">Home</NavLink>
                    <NavLink to="/about" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">About Us</NavLink>
                    <motion.div className="relative" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}>
                        <NavLink to="/services" style={({isActive}) => navLinkStyle({isActive: isActive || isServicesActive})} className="text-sm font-semibold hover:text-accent flex items-center gap-1 transition-colors">
                            Services
                        ...
                        <AnimatePresence>
                            {isDropdownOpen && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-56 bg-white rounded-md shadow-lg overflow-hidden"><div className="p-1">{servicesDropdownLinks.map(service => (<Link key={service.path} to={service.path} onClick={() => setIsDropdownOpen(false)} className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-primary rounded-md">{service.name}</Link>))}</div></motion.div>)}
                        </AnimatePresence>
                    </motion.div>
                    <NavLink to="/how-it-works" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">How It Works</NavLink>
                    <NavLink to="/why-outsource" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">Why Outsource</NavLink>
                    {/* <NavLink to="/technology" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">Technology</NavLink> */}
                    <NavLink to="/resources-faq" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">Resources & FAQ</NavLink>
                    <NavLink to="/contact" style={navLinkStyle} className="text-sm font-semibold hover:text-accent transition-colors">Contact Us</NavLink>
                </nav>
                <div className="flex items-center ml-8">
                    <div className="flex items-center space-x-4"> {socialLinks.map(link => ( <a key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" className="text-white hover:text-accent transition-colors"> <span className="sr-only">{link.name}</span> {link.icon} </a> ))} </div>
                    <div className="flex items-center space-x-2 pl-4 ml-4 border-l border-slate-700"> <img src="https://flagcdn.com/au.svg" width="20" alt="Australia Flag" title="Australia"/> <img src="https://flagcdn.com/in.svg" width="20" alt="India Flag" title="India"/> <img src="https://flagcdn.com/ca.svg" width="20" alt="Canada Flag" title="Canada"/> <img src="https://flagcdn.com/us.svg" width="20" alt="USA Flag" title="USA"/> </div>
                </div>
            </div>
          ...
          >
            <nav className="flex flex-col space-y-2">
              {allNavLinks.map(link => {
                  ...
                  return (
                    <div key={link.path}>
                      <button
                        ...
                        style={mobileNavLinkStyle(isServicesActive)}
                ...
                return (
                  <NavLink key={link.path} to={link.path} style={mobileNavLinkStyle} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-semibold px-4 py-3 rounded-md transition-colors">{link.name}</NavLink>
                );
              })}
            </nav>
            <div className="mt-auto pt-6 border-t border-slate-700 flex flex-col items-center">
                <div className="flex justify-center space-x-6"> {socialLinks.map(link => ( <a key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-white transition-colors"> <span className="sr-only">{link.name}</span> {link.icon} </a> ))} </div>
                <div className="flex items-center space-x-3 mt-6"> <img src="https://flagcdn.com/au.svg" width="24" alt="Australia Flag" title="Australia"/> <img src="https://flagcdn.com/in.svg" width="24" alt="India Flag" title="India"/> <img src="https://flagcdn.com/ca.svg" width="24" alt="Canada Flag" title="Canada"/> <img src="https://flagcdn.com/us.svg" width="24" alt="USA Flag" title="USA"/> </div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
